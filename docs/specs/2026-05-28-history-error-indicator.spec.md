...
| 原计划                                                                                                                                   | 实际实现                                                                                                                                                                                                                                                         | 原因                                                                                                                                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 扩展 `src/bindings.ts` 中 `HistoryEntry` 含 `error_summary: HistoryError \| null`，并同步 `src/lib/types.ts` 的 Zod schema（验收场景 7） | bindings.ts 没有 `HistoryEntry` 导出，`src/lib/types.ts` 也没有相应 Zod schema。仅更新前端独立类型 `src/components/settings/dashboard/dashboardTypes.ts`（加 `HistoryError` 接口 + `error_summary?: HistoryError \| null` 字段）                                 | 历史记录通过 `invoke<HistoryEntry[]>()` 强转，类型由前端本地定义，从未通过 specta 导出；spec 在 §约束/验收场景 7 中误以为它在 bindings.ts                     |
| SELECT + JOIN 仅供 `get_history_entries_paginated` 使用                                                                                  | 在 `impl HistoryManager` 上抽出 `HISTORY_ENTRY_SELECT` / `HISTORY_ENTRY_JOIN` 常量及 `map_history_entry_row` 助手，5 个读路径全部复用（`get_history_entries`、`get_history_entries_paginated`、`get_entries_for_date`、`get_entries_by_ids`、`get_entry_by_id`） | spec 漏写 `HistoryEntry` 实际有 5 个构造位点；保持 SELECT/JOIN 单一定义，避免后续字段漂移                                                                     |
| ⚠️ 按钮在 `opacity-0 group-hover/card:opacity-100` 动作区内                                                                              | ⚠️ 按钮放在动作区外层 wrapper 中（**始终可见**），原有动作按钮仍保持 hover-only                                                                                                                                                                                  | 错误指示器只在 hover 时出现违背特性意图（用户需扫一眼就知道哪条录音降级了）；spec 未明确指定可见性，按用户价值兜底                                            |
| `errorsOnly` 开关只过滤列表                                                                                                              | 同时把展示的 `totalCount` 改为过滤后的数量（当 errorsOnly 开启）                                                                                                                                                                                                 | 仅靠列表过滤但总数仍显示 20 条会让用户困惑「为什么只看到 3 条」                                                                                               |
| 错误面板含「时间戳」字段                                                                                                                 | 面板含 stage / error_type / detail / model / timestamp，timestamp 用 `entry.timestamp * 1000` 经 `Date.toLocaleString()` 渲染                                                                                                                                    | 与验收场景 2 一致；timestamp 复用 history 行的时间戳而非 `pipeline_decisions.timestamp`，因为后者未通过 JOIN 暴露到 frontend，且 history 时间戳同样能定位事件 |
