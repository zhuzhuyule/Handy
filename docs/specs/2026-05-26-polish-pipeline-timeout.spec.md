...
| 原计划                                                     | 实际实现                        | 原因                                                                                                                                                                                                       |
| ---------------------------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PER_CALL_TIMEOUT_SECS = 5` / `PIPELINE_TIMEOUT_SECS = 10` | 2026-05-28 调整为 **10s / 20s** | 5s 对正常网络下含连接 + body 等待的较长输出过于激进，用户反馈 20 字以内 5-10s 属正常区间。外层 ceiling 同步抬到 `2 × PER_CALL`，保留 fallback 重试预算。所有 spec 内出现的 "5s" / "10s" 数字以新常量为准。 |
