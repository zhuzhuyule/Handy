...
| 原计划                                                                                        | 实际实现                                                                                                                                  | 原因                                                                                                                                           |
| --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `resolve_test_extras` 直接返回元组 `(Option<HashMap<...>>, Option<HashMap<...>>)`             | 添加了 `type TestExtras = (Option<HashMap<String, Value>>, Option<HashMap<String, String>>);` 别名作为返回类型                            | clippy `type_complexity` 在嵌套泛型上触发；alias 让签名可读且复用                                                                              |
| 7 个单元测试（5 个 + 1 个回归）                                                               | 实际 7 个，但组成不同：4 个核心规则 + 1 个 thinking 碰撞优先级 + 1 个 thinking-only 路径 + 1 个 `override_some_with_cached_model_id` 回归 | 一轮 code review 反馈"merge collision"和"(Some(tp), None) 分支无覆盖"是 load-bearing 行为，必须显式锁定                                        |
| 计划假定 `cargo build` 即触发 specta 重生成 `src/bindings.ts`                                 | 实际需要运行 `bun tauri dev`（即真正启动 app），specta export 才会被调用，因为它在 `#[cfg(debug_assertions)]` 的 `pub fn run()` 内执行    | 项目把 export 放在 app 启动路径而非 build script                                                                                               |
| `handleAdd` 失败时设置 `failed` test state 显示错误                                           | catch 分支保留但实际不可达：`addCachedModel`（在 `settingsStore.ts:1068`）吞 error 仅 `console.error`，不抛出                             | 沿用现有 store 行为；修复 store 不在本 spec 范围。catch 留作 defensive guard 兼顾未来 store 改写                                               |
| 没有规定"跳过测试后又编辑 ID"的行为                                                           | 在 modelId `onChange` 中显式 reset `skipped` 和 passed/failed test state，让用户重新走 ping 流程                                          | 最终 review 指出"skipped 状态会让未测试的 ID 也能 Add"是 UX 漏洞；1 行修复，保证 spec §决策"测试通过 或 跳过"的语义不被 ID 编辑绕过            |
| 计划目标 ~250 行 `CustomAddModelDialog.tsx`                                                   | 实际 ~460 行                                                                                                                              | 完整状态机（4 态 TestState）+ Skip 链 + 重复 ID 警告 + handleAdd 持久化 + KeyValueEditor refs/count + thinking 预设按钮叠加，单文件可读性更好  |
| 计划 `Task 12` 用 `bun lint` 检查 frontend                                                    | 实际用 `bun run build`（即 `tsc && vite build`），因为 `package.json` 中没有独立 lint script                                              | 项目格式化走 prettier，类型检查走 tsc，没有 ESLint                                                                                             |
| 文字 fallback (`Body 参数`、`Headers`、`启用思考`、`禁用思考`、`测试`、`跳过测试直接添加` 等) | 大量字符串保留为硬编码中文，未提取到 `src/i18n/locales/*/translation.json`                                                                | 沿用 `EditModelDialog.tsx` 已有的硬编码中文模式（"Body 参数"、"Headers"、"启用思考" 等都已是硬编码）；i18n 全局拉齐为独立后续工作              |
| Thinking 预设按钮写入 `{ thinking: { type: "enabled" } }` 即可                                | 实际只写 Anthropic 风格，未做 provider 推断（OpenAI 用 `reasoning_effort`，Qwen 用 `enable_thinking`）                                    | spec §决策明确"用户通过预设按钮显式管理 thinking"；provider-aware 预设是 EditModelDialog.tsx 的复杂度，未来若有需求再单独跟进                  |
| Task 9 计划用 `as CachedModel` 强制断言                                                       | 实际用结构化对象字面量直接赋给 `const newModel: CachedModel`，无 `as`                                                                     | `extra_params: Record<string, unknown> \| undefined` 和 `extra_headers: Record<string, string> \| undefined`（types.ts:153-155）能直接匹配类型 |
| `useSettings().testPostProcessInference` 返回类型仅 `{content?, reasoning_content?}`          | 同步扩展为 `{content?, reasoning_content?, duration_ms?, total_tokens?}`                                                                  | 后端 `InferenceResult` 一直返回 4 字段，原 TS 类型不完整；扩展让 `testInferenceInline` 能取到 speed 信号                                       |
