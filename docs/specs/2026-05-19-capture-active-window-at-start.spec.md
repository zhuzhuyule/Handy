...
| 原计划                                                                                               | 实际实现                                                                                                                                                                | 原因                                                                            |
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Task 1 仅引入 slot + helpers + 测试                                                                  | 新 helpers (`store_start_active_window`, `take_start_active_window`, `clear_start_active_window`) 临时加上 `#[allow(dead_code)]`，Task 2/3/4 接线后逐步移除；末态无残留 | CLAUDE.md 强制要求提交前消除所有 warning，未引用的新符号会触发 `dead_code` lint |
| 测试代码包含计划示例中的中文行内注释（如 `// 先确保 slot 干净`、`// 第二次 take 同 id 应得到 None`） | 实际写入的测试省略了这两条行内注释                                                                                                                                      | 风格简化，不影响断言与行为；测试覆盖与计划一致                                  |
